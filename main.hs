...
unify (V c) x = get >>= \env -> if member c env then unify (env!c) x
        else modify (insert c x) >> return True
...
prolog t1 t2 = let (a,s) = runState (unify t1 t2) empty in if a then Just (toList s) else Nothing
